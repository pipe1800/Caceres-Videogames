...
export const categories = [
	...
	},
	{
		...
		subcategories: ['Accesorios Varios', 'Accesorios iPhone'],
	},
];
...
const Navigation = ({ onCategorySelect }: NavigationProps) => {
	...
	return (
		<nav className="bg-white border-b-2 border-[#3bc8da] shadow-sm hidden md:block">
			<div className="container mx-auto px-4">
				<div className="flex items-center justify-center py-3 sm:py-4">
					<ul className="flex flex-nowrap overflow-x-auto no-scrollbar gap-3 sm:gap-6 w-full justify-between sm:justify-center">
						{/* Home Button */}
						<li className="flex-shrink-0">
							<button
								onClick={handleHomeClick}
								className="flex items-center gap-2 px-3 sm:px-4 py-2 text-[#091024] hover:text-[#d93d34] font-medium transition-colors rounded-lg hover:bg-gray-50"
							>
						...
						{categories.map((category) => (
							<li key={category.name} className="relative group flex-shrink-0">
								<button
									className="flex items-center gap-2 px-3 sm:px-4 py-2 text-[#091024] hover:text-[#d93d34] font-medium transition-colors rounded-lg hover:bg-gray-50"
								...
								>
									...
									{category.subcategories.length > 0 && <ChevronDown className="w-4 h-4" />}
								</button>
